...
import { initMathCalculatorScene, destroyMathCalculatorScene } from "@/scripts/math-calculator";
...
export function MathCalculatorGame({ gameId, questions, onGameComplete }: MathCalculatorGameProps) {
  ...
  // Canvas banner
  useEffect(() => {
    if (gameStarted) initMathCalculatorScene({ canvasId: "math-banner" });
    return () => destroyMathCalculatorScene();
  }, [gameStarted]);
  ...
  const saveGameResult = async (finalScore: number, maxScore: number, timeTaken: number) => {
    try {
      ...
      // find/create game
      let { data: game } = await supabase.from("game").select("id").eq("slug", gameId).single();
      if (!game) {
        const { data: newGame } = await supabase.from("game").insert({
          slug: gameId,
      ...
      // insert play
      const { data: play } = await supabase.from("game_plays").insert({
        user_id: user.id,
        game_id: game.id,
        score: finalScore,
      }).select("id").single();
      ...
      const detailedScores = questions.map((q, index) => ({
        ...
        question_id: q.id,
        points: q.points,
        is_correct: Math.abs(parseFloat(answers[index] || "0") - parseFloat(q.correct_answer)) < 0.01,
      }));
      await supabase.from("game_scores").insert(detailedScores);

      // update user_totals
      const { data: existingTotal } = await supabase.from("user_totals").select("total_score,total_plays").eq("user_id", user.id).single();
      if (existingTotal) {
        await supabase.from("user_totals").update({
  ...
  if (!gameStarted) return (
    <div className="text-center space-y-6">
      <canvas id="math-banner" className="w-full h-48 rounded-lg overflow-hidden"></canvas>
              ...
              {["C", "/", "*", "-", "7", "8", "9", "+", "4", "5", "6", "1", "2", "3", "0", ".", "="].map(
                (val) => (
                  <Button
                    ...
                    variant="outline"
                    className={
                      val === "C" ? "bg-red-50 hover:bg-red-100" : ["+", "-", "*", "/", "="].includes(val) ? "bg-blue-50 hover:bg-blue-100" : ""
                    }
                  >
                    {val}
                  </Button>
