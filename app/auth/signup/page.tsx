...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!avatarUrl) {
      setError("Bạn phải chọn một avatar")
      return
    }
    ...
    try {
      ...
      if (!data?.user?.id) throw new Error("Đăng ký thất bại")

      const userId = data.user.id

      // 2️⃣ Tạo profile với avatar đã chọn
      const res = await fetch('/api/internal/create-profile', {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(profileData.error || "Lưu profile thất bại")
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-yellow-50 via-pink-50 to-sky-50">
