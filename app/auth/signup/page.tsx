  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { full_name: fullName, role }, // KHÔNG gửi avatar lúc này
        },
      })
      ...
      if (data?.user?.id) {
        // Signup thành công → chuyển tới trang chọn avatar, truyền userId
        router.push(`/auth/select-avatar?userId=${data.user.id}`)
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        <Card className="shadow-2xl border-0 rounded-3xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-gray-600 mt-2">Đăng ký tài khoản mới</CardDescription>
          </CardHeader>
